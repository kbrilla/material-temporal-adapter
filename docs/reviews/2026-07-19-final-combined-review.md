...
*Update 2026-07-20:* the vendor audit (`2026-07-20-vendor-demo-audit.md` §4) traced these claims to the old `temporal-adapter-demo` repo, which really does run Storybook test-runner + Playwright in CI — the claims are a migration remainder, not fabrication. Preferred resolution upgraded from "delete the claims" to "port the old demo's `test-storybook:ci` pattern" (the monorepo's story `play` functions are already runner-compatible).
...
| **D6** | File upstream Material issue/PR for navigation clamp + 13-month year view? | **Yes, after F1 ships here** | Material already clamps day-of-month in year/multi-year *selection* but not in header/keyboard *navigation* — a small consistency PR helps every strict adapter (Luxon reject modes too). The 12-cell year view is a second, separate upstream limitation worth an issue referencing #25753/#33276. Neither blocks anything in this repo. *Confirmed 2026-07-20:* a byte-level audit of the old demo's vendored Material build (`2026-07-20-vendor-demo-audit.md` §2) proves no pre-existing picker patch exists anywhere — the upstream fix must be authored from scratch. The same audit shows PR #32668's adapter carries the F1–F5 defect classes too; cross-reference this repo's fixes on that PR. |
